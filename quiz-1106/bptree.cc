...
insert_in_parent(NODE *left_child, int rs_key, NODE *right_child)
{
    ...
    TEMP t;
    copy_from_left_to_temp(&t, left_parent);
    insert_temp_after_left_child(&t, left_child, rs_key, right_child);
    erase_entries(left_parent);
    right_parent = alloc_internal(left_parent->parent);
    copy_from_temp_to_left_parent(&t, left_parent);
    int key2 = t.key[(int)ceil(N/2)];
    copy_from_temp_to_right_parent(&t, right_parent);
    insert_in_parent(left_parent, key2, right_parent);
	}
}
